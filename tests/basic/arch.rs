...
pub fn EXTZ(m: i128, v: BitDynamic) -> BitDynamic {
    sail_zero_extend(core_ctx, v, m)
}

/// EXTS
///
/// Generated from the Sail sources at `tests/basic/arch.sail` L10.
pub fn EXTS(m: i128, v: BitDynamic) -> BitDynamic {
    sail_sign_extend(v, m)
}

/// xlen
///
/// Generated from the Sail sources at `tests/basic/arch.sail` L14.
pub const xlen: i128 = 64;

/// xlen_bytes
///
/// Generated from the Sail sources at `tests/basic/arch.sail` L15.
...
pub fn execute(core_ctx: &mut Core, merge_hashtag_var: ast) {
    match merge_hashtag_var {
        ast::ITYPE((imm, rs1, rd, iop::RISCV_ADDI)) => {let rs1_val: BitStatic::<64> = rX(core_ctx, rs1);
