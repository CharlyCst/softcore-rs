...
pub fn sign_extend(n: i128, input: BitVector) -> BitVector {
    sail_sign_extend(input, n)
}

pub fn sub_vec(a: BitVector, b: BitVector) -> BitVector {
...
pub fn shift_bits_left(value: BitVector, shift: BitVector) -> BitVector {
    bv(value.len(), value.bits() << shift.bits())
}

pub fn shift_bits_right(value: BitVector, shift: BitVector) -> BitVector {
    bv(value.len(), value.bits() >> shift.bits())
}

pub fn shift_right_arith(value: BitVector, shift: i128) -> BitVector {
    ...
    let sign_bit = (value.bits() >> (value.len() - 1)) & 1;

    // If shift is >= N, all bits become the sign bit
    if shift >= value.len() {
        if sign_bit == 0 {
            return bv(value.len(), 0);
        } else {
